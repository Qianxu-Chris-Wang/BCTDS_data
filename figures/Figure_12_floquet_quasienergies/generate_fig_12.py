...
Single-column, 3-row figure:
  ...
  (b)  log10 FFT{|ring-down|} vs FFT freq × drive freq
       (FFT taken over *ring-down only*: t > 0)
  (c)  Floquet quasi-energies vs drive freq

Portability changes:
  • Removed process-based parallelism.
  • Default: single-threaded sweep (most portable).
  • Optional: Windows-friendly ThreadPoolExecutor (set USE_PARALLEL=True).
"""
...
# Windows-friendly parallel controls (threads often OK with NumPy/QuTiP)
USE_PARALLEL  = False   # set True to enable threads
MAX_WORKERS   = 12      # only used if USE_PARALLEL=True
...
def simulate_point(omega_d):
    """
    Run one drive frequency point.
    Returns:
      pop:  population ⟨σ⁺σ⁻⟩(t)
      quasi: array of 4 quasi-energies (GHz), NaN on failure
    """
    def drive_coeff(t, args):
        # Square-cosine pulse of duration t_drive_ns (kept as in your code)
        if t <= t_drive_ns:
...
if __name__ == "__main__":
    ...
    if USE_PARALLEL:
        with ThreadPoolExecutor(max_workers=min(MAX_WORKERS, n_freq)) as pool:
            futures = {pool.submit(simulate_point, w): i for i, w in enumerate(omega_d_vals)}
            for fut in tqdm(as_completed(futures), total=n_freq, desc="drive sweep (threads)"):
                idx = futures[fut]
                pop_row, q_row = fut.result()
                heat[:, idx] = pop_row
                quasi[idx]    = q_row
    else:
        for i, w in enumerate(tqdm(omega_d_vals, desc="drive sweep")):
            pop_row, q_row = simulate_point(w)
            heat[:, i] = pop_row
            quasi[i]   = q_row

    # ─────────────────────── ring-down FFT (panel b data) ───────────────
    # Take FFT over ring-down portion only: t > 0 (unchanged).
    mask_tail     = t_ns > 0
    ...
    im = ax_heat.imshow(
        heat,
        extent=[omega_d_vals[0], omega_d_vals[-1], t_us[0], t_us[-1]],
    ...
    ax_heat.text(-0.12, 1.0, r"(a)", transform=ax_heat.transAxes,
                 fontsize=69, fontweight="bold")
    ...
    cb_fft.set_label(r"Log$_{10}$ (FFT$(\langle \sigma^{+}\sigma^{-} \rangle))$ (arb.)",
                     fontsize=48, labelpad=20)
    cb_fft.ax.tick_params(labelsize=45)
    ax_fft.text(-0.12, 1.0, r"(b)", transform=ax_fft.transAxes,
                fontsize=69, fontweight="bold")
    ...
    ax_quasi.text(-0.12, 1.08, r"(c)", transform=ax_quasi.transAxes,
                  fontsize=69, fontweight="bold")
    ...
    outfile = os.path.join(outdir, "floquet_ringdown_fft_column.png")
