...
def fit_piecewise_slider(trace_list, time_spacing_us, show_slider=True):
    ...
    def update(val):
        ...
        title.set_text(f"Trace at {3900 + idx:.1f} MHz")
...
# find_FWHM_and_plot(pulse_width=308, save_plot=True)
...
# pulse_start_idx = 20 #11
# pulse_end_idx = 28
line_plot_line_width = 0.8
pulse_mark_line_width = 1.5
pulse_mark_line_color = 'black'
pulse_mark_line_style = '--'
display_freq_range = [3.9, 4.4]
display_time_range = [0, 1.6]
display_time_ticks = [0, 0.4, 0.8, 1.2, 1.6]
# display_time_range = [0, 0.4]
# display_time_ticks = [0, 0.4]
display_tau_range = [50,250]
display_tau_ticks = [100, 200]
display_S21_range = [3.13, 3.35]
display_S21_ticks = [3.2, 3.3]
...
npy_name_prefix = "Al2O3_200"
display_offset = 25
pulse_frequency_list = np.arange(3900, 4400, 1).tolist()

pulse_start_idx = 17 #11
pulse_end_idx = 27

mag_avg_matrix = np.load(os.path.join(matrix_data_dir, f"{npy_name_prefix}_mag_avg_matrix.npy"))
mag_avg_log_matrix = np.log10(mag_avg_matrix + 0.01)
mag_avg_log_matrix = mag_avg_log_matrix[:, display_offset:display_offset+950]
...
ax_left.text(abcd_x, abcd_y, r'(a)', transform=ax_left.transAxes,
             fontsize=abcd_size, fontweight='bold', va='top', ha='right', color='black')

ax_left.text(i_x, i_y, r'(i)', transform=ax_left.transAxes,
...
ax_top.set_xlim(display_S21_range)  # adjust as needed
ax_top.set_xticks(display_S21_ticks) 
...
npy_name_prefix = "Al2O3_500"
display_offset = 25
pulse_frequency_list = np.arange(3900, 4400, 1).tolist()

pulse_start_idx = 0 #11
pulse_end_idx = 28
...
ax_left.tick_params(axis='x', colors=tau_color)
ax_left.spines['bottom'].set_color(tau_color)
ax_left.text(abcd_x, abcd_y, r'(b)', transform=ax_left.transAxes,
             fontsize=abcd_size, fontweight='bold', va='top', ha='right', color='black')
...
center = 4.213
band_width = find_FWHM_and_plot(pulse_width=500, save_plot=False)
pick_time=1.5
ax_right.vlines(x=pick_time, ymin=center - band_width/2, ymax=center + band_width/2, color='white', linewidth=1.5)

ax_right.text(
    text_box_x, text_box_y, "50 ns pulse",  # position in axes coordinates
    transform=ax_right.transAxes,
    fontsize=text_box_font_size,
    ha='right', va='bottom',
    color='black',
...
ax_left.tick_params(axis='x', colors=tau_color)
ax_left.spines['bottom'].set_color(tau_color)
ax_left.text(abcd_x, abcd_y, r'(c)', transform=ax_left.transAxes,
             fontsize=abcd_size, fontweight='bold', va='top', ha='right', color='black')
...
center = 4.213
band_width = find_FWHM_and_plot(pulse_width=1950, save_plot=False)
pick_time=1.5
ax_right.vlines(x=pick_time, ymin=center - band_width/2, ymax=center + band_width/2, color='white', linewidth=1.5)

ax_right.text(
    text_box_x, text_box_y, "200 ns pulse",  # position in axes coordinates
    transform=ax_right.transAxes,
    fontsize=text_box_font_size,
    ha='right', va='bottom',
    color='black',
...
file_path = os.path.join(fig_save_dir, f"Fig_pulse_width.png")
if save_fig:
