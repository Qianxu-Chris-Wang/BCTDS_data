...
# Optional Windows-friendly parallelism (threads); set USE_PARALLEL=True below
...
# Parallelism controls (Windows-friendly):
USE_PARALLEL        = False   # <- set True to enable thread-based parallel sweep
MAX_WORKERS         = 16      # used only if USE_PARALLEL is True
...
def _simulate_one(f_drv):
    ...
    return run_simulation_for_frequency(
        f_drv, tlist, init_freqs, H_int, GAMMA, GAMMA_PHI, DRIVE_AMPL, _simulate_one.pulse_ns
    )

def run_sweep_for_pulse(pulse_ns):
    """
    Returns:
      pop_tr: shape (len(FREQ_AXIS), len(tlist)) population trajectories
    """
    _simulate_one.pulse_ns = pulse_ns  # attach current pulse to the callable

    pop_tr = np.zeros((len(FREQ_AXIS), len(tlist)))
    desc   = f"pulse {pulse_ns:4.1f} ns"

    if USE_PARALLEL:
        # Windows-friendly threads (NumPy/QuTiP heavy ops often release the GIL)
        with ThreadPoolExecutor(max_workers=min(MAX_WORKERS, len(FREQ_AXIS))) as pool:
            futures = {pool.submit(_simulate_one, f_drv): i for i, f_drv in enumerate(FREQ_AXIS)}
            for fut in tqdm(as_completed(futures), total=len(futures), desc=desc):
                pop_tr[futures[fut]] = fut.result()
    else:
        # Portable, deterministic single-threaded path
        for i, f_drv in enumerate(tqdm(FREQ_AXIS, desc=desc)):
            pop_tr[i] = _simulate_one(f_drv)
...
if __name__ == "__main__":
    ...
    for row, pulse_ns in enumerate(PULSES_NS):
        ...
        # sweep (now portable)
        pop_tr = run_sweep_for_pulse(pulse_ns)

        # FFT of ring-down part
        mask_tail     = tlist > 0  # ignore drive pulse (keep as in your original)
    ...
    outfile = os.path.join(outdir, f"NTLS_{N_TLS}_ringdown_fft_all_pulses_{datetime.datetime.now():%Y%m%d_%H%M%S}.png")
